  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 pb-28 max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black tracking-tight text-white dark:text-white light:text-slate-900 flex items-center space-x-2">
        ...
        {historyList.length > 0 && (
          <button
            ...
            className="px-3.5 py-2 rounded-2xl bg-neon/15 border border-neon/30 text-neon font-bold text-xs flex items-center space-x-1.5 shadow-sm active:scale-95 transition-all hover:bg-neon hover:text-black"
          >
            <Download size={14} />
      ...
      <div className="grid grid-cols-2 gap-3">
        {/* Total Minutes */}
        <div className="glass-panel p-4 rounded-3xl border border-white/10 relative overflow-hidden">
          <div className="flex items-center space-x-1.5 text-[11px] font-bold text-gray-400 uppercase">
        ...
        <div className="glass-panel p-4 rounded-3xl border border-white/10 relative overflow-hidden">
          ...
          <div className="mt-2 flex items-baseline space-x-1">
            <span className="font-mono text-2xl sm:text-3xl font-black text-neon">
              {stats.totalCalories}
            </span>
        ...
        <div className="glass-panel p-4 rounded-3xl border border-white/10 relative overflow-hidden">
          ...
          <div className="mt-2 flex items-baseline space-x-1">
            ...
            <span className="text-xs text-gray-400 font-bold">buổi</span>
          </div>
          <div className="text-[10px] text-gray-500 mt-1">
            Đã hoàn thành
          </div>
      ...
      <div className="space-y-3">
        ...
        {historyList.length === 0 ? (
          <div className="text-center py-12 glass-panel rounded-3xl p-6 space-y-3">
            ...
            <h4 className="text-sm font-bold text-gray-300">Chưa có lịch sử tập luyện</h4>
            <p className="text-xs text-gray-500 max-w-xs mx-auto">
              Hãy hoàn thành bài tập đầu tiên của bạn để xem biểu đồ và số liệu phân tích tại đây!
            </p>
            <button
              onClick={onStartWorkout}
              className="mt-2 px-5 py-2.5 rounded-2xl bg-neon text-black font-bold text-xs uppercase tracking-wider shadow-neon active:scale-95 transition-all"
            >
              Bắt Đầu Tập Ngay
            </button>
          </div>
          ...
          <div className="space-y-2.5">
            {historyList.map((item) => (
              <div
                key={item.id}
                className="glass-panel p-4 rounded-2xl border border-white/5 flex items-center justify-between hover:border-white/20 transition-all"
              >
                <div className="flex items-start space-x-3">
                  <div className="w-10 h-10 rounded-2xl bg-neon/10 border border-neon/20 flex items-center justify-center text-neon shrink-0 mt-0.5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 text-[10px] text-gray-500 mt-1">
                      ...
                      {item.completedSets && (
                        <>
                          <span>•</span>
                          <span className="text-gray-400 font-semibold">{item.completedSets} Hiệp</span>
                        </>
                      )}
                    </div>
                  </div>
